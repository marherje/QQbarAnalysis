...
d point (in m), x, y, z " << winner2->getEndpoint()[0]
  ...
  int QQbarMCOperator::getID(MCParticle *winner)
  {
  ...
  vector<MCParticle *> QQbarMCOperator::getParents(MCParticle *winner)
  {

    vector<MCParticle *> parents(0);
    if (winner == NULL)
      return parents;
    return winner->getParents();
  }
