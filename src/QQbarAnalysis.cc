...
namespace QQbarProcessor
{
	...
	bool QQbarAnalysis::WritePFOInfo(LCEvent *evt, ReconstructedParticle *component, int pfo_recorded, int ijet, int ivtx, std::string _colName, std::string _versionPID)
	{
		...
		if (component->getClusters().size() > 0)
		{
			streamlog_out(DEBUG) << " _stats._pfo_E_calo=" << component->getClusters()[0]->getEnergy();
		}
		...
		for (int i = 0; i < 3; i++)
		{
			_stats._pfo_vtxpt[pfo_recorded][i] = trk->getReferencePoint()[i];
			if (component->getClusters().size() > 0)
			{
				_stats._pfo_endpt[pfo_recorded][i] = component->getClusters()[0]->getPosition()[i];
			}
		}
	...
	void QQbarAnalysis::PFOCheatInfo(ReconstructedParticle *component, QQbarMCOperator &operaMC, std::vector<MCParticle *> isr_stable, int pfo_recorded)
	{
		...
		_stats._pfo_pdgcheat_id[pfo_recorded] = operaMC.getID(mctrack);
		streamlog_out(DEBUG) << " PDG CHEAT ID " << _stats._pfo_pdgcheat_id[pfo_recorded] << std::endl;

		vector<MCParticle *> parents = operaMC.getParents(mctrack);
		_stats._pfo_nparents[pfo_recorded] = parents.size();
		streamlog_out(DEBUG) << " nparents : " << _stats._pfo_nparents[pfo_recorded] << " : ";
		for (int iparent = 0; iparent < parents.size(); iparent++)
		{
			_stats._pfo_pdgcheat_parent[pfo_recorded][iparent] = parents.at(iparent)->getPDG();
			streamlog_out(DEBUG) << _stats._pfo_pdgcheat_parent[pfo_recorded][iparent] << " ";
		}
		...
		try
		{
			...
			else
			{
				...
				try
				{
					...
					if(!minoraxis.empty())
					{
						_stats._minor_thrust_axis[0] = minoraxis.at(0);
						_stats._minor_thrust_axis[1] = minoraxis.at(1);
						_stats._minor_thrust_axis[2] = minoraxis.at(2);
					}
					...
					if(!majoraxis.empty())
					{
						_stats._major_thrust_axis[0] = majoraxis.at(0);
						_stats._major_thrust_axis[1] = majoraxis.at(1);
						_stats._major_thrust_axis[2] = majoraxis.at(2);
					}

					std::vector<float> principleaxis;
					jetcol->getParameters().getFloatVals("principleThrustAxis", principleaxis);
					if(!principleaxis.empty())
					{
						_stats._principle_thrust_axis[0] = principleaxis.at(0);
						_stats._principle_thrust_axis[1] = principleaxis.at(1);
						_stats._principle_thrust_axis[2] = principleaxis.at(2);
					}
					...
					if(!sphericitytensor.empty())
					{
						_stats._sphericity_tensor[0] = sphericitytensor.at(0);
						_stats._sphericity_tensor[1] = sphericitytensor.at(1);
						_stats._sphericity_tensor[2] = sphericitytensor.at(2);
					}
				}
				catch (lcio::DataNotAvailableException e)
				{
			...
			for (int ijet = 0; ijet < 2; ijet++)
			{
				...
				for (int i = 0; i < components_originalPFO.size(); i++)
				{
					...
					if (record_pfo == true)
					{
						...
						streamlog_out(DEBUG) << " IS not a secondary TRACK: save info " << std::endl;
						LCCollection *tjmcplcol = evt->getCollection("TrueJetPFOLink");
						LCRelationNavigator navigator(tjmcplcol);
						vector<LCObject *> true_obj = navigator.getRelatedFromObjects(component);
						streamlog_out(DEBUG) << " TRUEJETS TEST navigator size: " << true_obj.size() << "\n";
				...
				for (int ivtx = 0; ivtx < vertices->size(); ivtx++)
				{
					Vertex *ivertex = vertices->at(ivtx);
					...
					const float * vtx_pos = ivertex->getPosition();
					float vtx_d0 = sqrt( vtx_pos[0]*vtx_pos[0] + vtx_pos[1]*vtx_pos[1] );

					_stats._vtx_d0[ijet][ivtx] = vtx_d0;
					_stats._vtx_z0[ijet][ivtx] = vtx_pos[2];
